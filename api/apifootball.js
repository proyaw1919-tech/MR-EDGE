...
export default async function handler(req,res){
  ...
  if(!key)return res.status(200).json({injuries:null,prediction:null});
  const{home,away,date,league}=req.query;
  if(!home||!away||!date||!league)return res.status(200).json({injuries:null,prediction:null});
  const leagueId=LEAGUE_IDS[league];
  if(!leagueId)return res.status(200).json({injuries:null,prediction:null});
  try{
    ...
    if(!fix)return res.status(200).json({injuries:null,prediction:null});
    ...
    return res.status(200).json({injuries,prediction});
  }catch{return res.status(200).json({injuries:null,prediction:null});}
}
