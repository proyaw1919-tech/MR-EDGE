...
`;
        ...
        // Card averages (yellow+red) from API-Football (last ~7 matches per team)
        const crd = apiFootball?.cards;
        if (crd && (crd.home || crd.away)) {
          const kLines: string[] = [];
          if (crd.home) kLines.push(`${match.home}: gets ${crd.home.for} cards/game, opponents get ${crd.home.against}/game (last ${crd.home.games} matches)`);
          if (crd.away) kLines.push(`${match.away}: gets ${crd.away.for} cards/game, opponents get ${crd.away.against}/game (last ${crd.away.games} matches)`);
          if (crd.home && crd.away) {
            const expTotal = (crd.home.for + crd.away.for) / 1; // both teams' own cards summed
            kLines.push(`Statistical expected total cards: ${expTotal.toFixed(1)} (${match.home} ~${crd.home.for}, ${match.away} ~${crd.away.for})`);
          }
      ...
      {cards && (cards.totalCards != null || cards.recommendation) && (
        <div style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 8, marginBottom: 8 }}>
            <div style={{ padding: "12px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: "#8a8a8a", marginBottom: 4 }}>{match.home}</div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#fff", fontFamily: "inherit" }}>{cards.homeCards ?? "–"}</div>
            </div>
            <div style={{ padding: "12px 10px", background: "rgba(250,204,21,0.1)", border: "1px solid rgba(250,204,21,0.35)", borderRadius: 6, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: "#8a8a8a", marginBottom: 4 }}>TOTAL</div>
              <div style={{ fontSize: 22, fontWeight: 800, color: "#facc15", fontFamily: "inherit" }}>{cards.totalCards ?? "–"}</div>
            </div>
            <div style={{ padding: "12px 10px", background: "rgba(255,255,255,0.04)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: 6, textAlign: "center" }}>
              <div style={{ fontSize: 10, color: "#8a8a8a", marginBottom: 4 }}>{match.away}</div>
